...
> See [product.md](./product.md) for requirements, [roadmap.md](./roadmap.md) for phasing.
...
Base path `/api/v1`. JSON. JWT bearer auth (single operator role for MVP). Conventions in [standards/api_rules.md](../standards/api_rules.md).
...
| POST | `/auth/login` | Get JWT | Planned |
| GET | `/clinics` | Ranked, searchable, filterable list | ✅ |
| GET | `/clinics/{id}` | Clinic detail (profile + signals + score + enrichment) | ✅ |
| POST | `/clinics/discover` | Trigger discovery for a query/region | ✅ |
| POST | `/clinics/{id}/signals:detect` | Run signal detection for a clinic | ✅ |
| GET | `/clinics/{id}/signals` | List signals | ✅ |
| POST | `/clinics/{id}/score` | (Re)compute score | ✅ |
| POST | `/clinics/{id}/enrich` | Run AI enrichment (`?force=true` optional) | ✅ |
| GET | `/scoring-config` | Get active config | ✅ |
| PUT | `/scoring-config` | Update weights/bands (re-score on demand) | ✅ |
...
│                                 SignalList, AIBreakdown, ScoreBreakdown, ui/
...
Production stack: `docker-compose.prod.yml` + `.env.production`. Runbooks: [runbooks/deploy.md](../runbooks/deploy.md), [runbooks/rollback.md](../runbooks/rollback.md).
...
| `/api/v1/health/ready` | Readiness — DB reachable after migrations |
...
### CI/CD
- **ci.yml:** ruff + pytest + eslint + vitest on PR/push
- **deploy.yml:** on `main` → test → build/push GHCR images → smoke deploy + rollback script
...
- **Open API (known gap):** mutating/expensive routes (`PUT /scoring-config`, `POST /clinics/discover`, `/{id}/enrich`, `/{id}/signals:detect`) are unauthenticated in MVP. Until JWT auth ships (post-MVP), bind the API to an internal network (reverse proxy + VPN/allowlist), **not** public `0.0.0.0`, and add edge rate limiting.
