...
- Next: Pilot with real data; wire auth + rate limiting before any public exposure.
...
- Decisions: Client-side fetch via `NEXT_PUBLIC_API_URL` for browser; `API_URL` for server-side in Docker; priority-colored badges aligned to scoring bands.
