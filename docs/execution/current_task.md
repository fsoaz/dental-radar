...
4. `python cli.py detect --all && python cli.py score --all && python cli.py enrich --all`
5. Open http://localhost:3000/clinics — filter Hot/Immediate, review detail pages
6. Tune scoring via `PUT /api/v1/scoring-config` if needed
