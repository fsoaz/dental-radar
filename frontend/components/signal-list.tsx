...
import { formatDetectedAt, isStaleSignal } from "@/lib/utils";
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {signals.map((signal) => {
          const age = formatDetectedAt(signal.detected_at);
          const stale = isStaleSignal(signal.detected_at);
          return (
            <div key={`${signal.type}-${signal.evidence ?? "none"}`} className="rounded-md border p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {age ? (
                  <span className={`text-sm ${stale ? "text-amber-700" : "text-muted-foreground"}`}>
                    {age}
                    {stale ? " (stale)" : ""}
                  </span>
                ) : null}
